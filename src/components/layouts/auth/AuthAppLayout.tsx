	...
	return (
		<Box w="100vw" h="100vh">
			...
			<Flex h="100vh" w="100vw" overflow="auto" bg="gray.50">
				{desktopMode && <Box w={isOpenSideBar ? '255px' : '99px'} mx="23px"> <Sidebar /> </Box>}
				<Box
					w={desktopMode ? `calc(100vw - 23px - ${isOpenSideBar ? '255px' : '99px'} - 17px)` : '95%'}
				>
					<Box mx="17px">
						<TopBar title={pageTitle} email={session?.data?.user?.email} />
					</Box>
					<Box h="calc(100vh - 113px)" overflow="auto" ml="17px" pr="17px" mt="20px" w="100%">
						{children}
					</Box>
				</Box>
			</Flex>
